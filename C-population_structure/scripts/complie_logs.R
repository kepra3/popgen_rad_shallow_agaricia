...
setwd("~/Dropbox/agaricia_project_2019/shalo_ag/gen_project/popgen_rad_shallow_agaricia/C-population_structure/results/")
for (vcf_name in c("ac_1diii_nc", "hu_1diii_nc", "lm_1diii_nc-wnr", "all-aga_1diii_nc-wnr")) {
               

  logs <- data.frame(V1 = integer(), V2 = double())
  for (i in 2:10) {
    loglikelihood <- read.table(paste0("admix_runs/20percent_", vcf_name, "_", i, "/loglikelihood.txt"), quote = "\"", comment.char = "")
  ...
  write.table(logs, file = paste0("admix_runs/logs_", vcf_name, ".txt"), col.names = FALSE, row.names = FALSE, quote = FALSE)
  ...
  ggsave(paste0("admix_plots/Log-likelihood_", vcf_name, ".pdf"), height = 7, width = 7, units = "cm")
  
  
  CVs <- data.frame(V1 = integer(), V2 = double())
  for (i in 2:10) {
    CV <- read.table(paste0("admix_runs/20percent_", vcf_name, "_", i, "/CV_error.txt"), quote = "\"", comment.char = "")
  ...
  ggplot(CVs, aes(x = V1, y = V4)) + geom_line(colour = "red") + geom_point() + theme_bw() +
    ...
    theme(axis.title.x = element_text(size = 12),
          ...
          axis.text.x = element_text(size = 8),
          axis.text.y = element_text(size = 8))
  ggsave(paste0("admix_plots/CV_error_", vcf_name, ".pdf"), height = 7, width = 7, units = "cm")
}
